    ...
    <div className='pt-28 md:pt-35 px-4 md:px-16 lg:px-24 xl:px-32'>
      ...
      <div className='flex flex-col items-start text-left mb-10'>
        ...
        <p className='text-sm md:text-base text-gray-500/90 mt-2 max-w-3xl'>
          Comfort. Style. Everything you need for a relaxing stay.
          Choose from cozy singles to elegant suites — all with top amenities and great service.
        </p>
      </div>

      {/* MAIN SECTION: Rooms + Filters */}
      <div className='flex flex-col lg:flex-row gap-8'>
        ...
        {/* FILTER BOX */}
        <div className='w-full lg:w-[25%] h-fit bg-white border border-gray-300 text-gray-600 p-4'>
          <div className='flex justify-between items-center mb-4'>
            <p className={`text-base font-medium text-gray-800 min-lg:border-b border-gray-300 
            ${openFilters && "border-b"}`}>Filters</p>
            <div className='text-xs cursor-pointer'>
              <span onClick={()=> setOpenFilters(!openFilters)} className='lg:hidden'>
                {openFilters? 'HIDE': 'SHOW'}
              </span>
              <span className='hidden lg:block text-sm text-blue-600 cursor-pointer'>Clear</span>
            </div>
          </div>
          <div className={`${openFilters? 'h-auto': 'h-0 lg:h-auto overflow-hidden transition-all duration-700'}`}>
            <div className='px-5 pt-5'>
              ...
              {roomTypes.map((room, index) => (
                ...
                onChange={(checked) => hadleFilterChange(checked, room, 'roomType')}
                />
              ))}
            </div>
            ...
            <div className='px-5 pt-5'>
              ...
              {priceRanges.map((range, index) => (
                ...
                onChange={(checked) => hadleFilterChange(checked, range, 'priceRange')}
                />
              ))}
            </div>
